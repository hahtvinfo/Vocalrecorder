...
static SLVolumeItf recorderVolume;

// synthesized sawtooth clip
#define SAWTOOTH_FRAMES 8000
static short sawtoothBuffer[SAWTOOTH_FRAMES];

// 300 seconds of recorded audio at 16 kHz mono, 16-bit signed little endian
#define RECORDER_FRAMES (16000 * 300)
...
static int64_t timeStart = -1;
static int64_t timeStop = 0;

static int64_t pauseStartIndices[20];
static int64_t pauseEndIndices[20];
static int pauseCount = 0;
static int64_t  recordStartTime = 0;
static SLmillisecond playTime = 0;
static int64_t  lastStartTime = 0;


#include <time.h>

#define NANOS_IN_SECOND 1000000000LL
#define NANOS_IN_MILISECOND 1000000LL
...
/* Callback for playing buffer queue events */
void PlayCallback(
        SLRecordItf caller,
        void *pContext,
        SLuint32 event)
{
    if (SL_PLAYEVENT_HEADATNEWPOS & event) {
        ...
        playTime = pMsec;
        lastStartTime = currentTimeInNanos();
    }
        ...
        SLuint32 event)
{
    if (SL_RECORDEVENT_HEADATNEWPOS & event) {
        if (timeStart < 0) {
            ...
            timeStart = currentTimeInNanos() - (int64_t)(pMsec*NANOS_IN_MILISECOND);
...
if (NULL != fdPlayerPlay) {
    ...
    // get duration
    SLmillisecond milis;
    result = (*fdPlayerPlay)->GetPosition(fdPlayerPlay, &milis);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;

    recordStartTime = milis - pMsec;
}
}*/
        }
    }
                                                             ...
                                                             jint count)
{
    ...
    switch (which) {
        ...
        case 4:     // CLIP_PLAYBACK
            nextBuffer = createResampledBuf(4, SL_SAMPLINGRATE_22_05, &nextSize);
                                                                         ...
                                                                         jobject assetManager, jstring filename)
{
    ...
    // thanglv
    result = (*fdPlayerPlay)->SetPositionUpdatePeriod(fdPlayerPlay, 100);
    assert(SL_RESULT_SUCCESS == result);
    result = (*fdPlayerPlay)->SetCallbackEventsMask(fdPlayerPlay,
                                                    SL_PLAYEVENT_HEADATMARKER | SL_PLAYEVENT_HEADATNEWPOS);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;
    ...
    // Reduce volume 2db
    result = (*fdPlayerVolume)->SetVolumeLevel(fdPlayerVolume, -2000);
    assert(SL_RESULT_SUCCESS == result);
    (void) result;
...
jboolean Java_com_example_nativeaudio_NativeAudio_createAudioRecorder(JNIEnv* env, jclass clazz)
{
    ...
    // thanglv
    result = (*recorderRecord)->SetPositionUpdatePeriod(recorderRecord, 100);
    assert(SL_RESULT_SUCCESS == result);
    ...
    result = (*recorderRecord)->RegisterCallback(recorderRecord, RecCallback, NULL);
    assert(SL_RESULT_SUCCESS == result);
    (void)result;
...
void Java_com_example_nativeaudio_NativeAudio_startRecording(JNIEnv* env, jclass clazz)
{
    ...
    for (int i = 0; i < 20; i ++){
        pauseStartIndices[i] = 0;
        pauseEndIndices[i] = 0;
    }

    // clear data
    memset(recorderBuffer, 0, RECORDER_FRAMES);
        ...
        jint count)
{
    ...
    SLmillibel maxLevel;
    (*bqPlayerVolume)->GetMaxVolumeLevel(bqPlayerVolume, &maxLevel);
    (*bqPlayerVolume)->SetVolumeLevel(bqPlayerVolume, maxLevel);
...
// set the playing state for the asset audio player
void Java_com_example_nativeaudio_NativeAudio_stopClip(JNIEnv* env,
                                                                         jclass clazz)
{
//    if (pthread_mutex_trylock(&audioEngineLock)) {
//        // If we could not acquire audio engine lock, reject this request and client should re-try
        ...
        jobject assetManager, jstring filename)
{
    ...
    // thanglv disable loop
    // enable whole file looping
    result = (*fdPlayerSeek)->SetLoop(fdPlayerSeek, SL_BOOLEAN_TRUE, 0, SL_TIME_UNKNOWN);

    // thanglv seek to 5000ms
    //result = (*fdPlayerSeek)->SetPosition (fdPlayerSeek, 15000, SL_SEEKMODE_ACCURATE);
    //assert(SL_RESULT_SUCCESS == result);
    //(void)result;
...
// set duration of the asset audio player
jint Java_com_example_nativeaudio_NativeAudio_getDurationAssetAudioPlayer(JNIEnv* env,
                                                                         jclass clazz)
{
    jint duration = 0;
    ...
    if (NULL != fdPlayerPlay) {
        ...
        // get duration
        SLmillisecond milis;
        result = (*fdPlayerPlay)->GetDuration(fdPlayerPlay, &milis);
        assert(SL_RESULT_SUCCESS == result);
        (void)result;
...
// set duration of the asset audio player
jint Java_com_example_nativeaudio_NativeAudio_getCurrentPositionAssetAudioPlayer(JNIEnv* env,
                                                                          jclass clazz)
{
    jint position = 0;
...
// set the recording state for the audio recorder
void Java_com_example_nativeaudio_NativeAudio_stopRecording(JNIEnv* env, jclass clazz)
{
//    if (pthread_mutex_trylock(&audioEngineLock)) {
//        return;
//    }
                                                                                 ...
                                                                                 jclass clazz)
{
    ...
    jint position = playTime + (currentTimeInNanos() - lastStartTime)/NANOS_IN_MILISECOND;
...
void Java_com_example_nativeaudio_NativeAudio_stopClip(JNIEnv* env,
                                                       jclass clazz) {
//    Java_com_example_nativeaudio_NativeAudio_selectClip(env, clazz, 1, 1);
...
void Java_com_example_nativeaudio_NativeAudio_stopRecording(JNIEnv* env, jclass clazz) {
    ...
    result = (*recorderRecord)->SetRecordState(recorderRecord, SL_RECORDSTATE_STOPPED);
    if (SL_RESULT_SUCCESS == result) {
        timeStop = currentTimeInNanos();
        recorderSize = (timeStop - timeStart)*22050 * sizeof(short)/1000000000LL;
        ...
        for (int i = 0; i < recorderSize/sizeof(short); i ++) {
            int absValue  = abs(recorderBuffer[i]);
            if (absValue > maxLevel){
                greaterCount ++;

                if (greaterCount > 1000) {
                    maxLevel = absValue;
                    greaterCount = 0;
                }
            }
        ...
        float gain = 20000.0/maxLevel; //32767.0/maxLevel;
        if (gain < 1.0){
            gain = 1.0;
        }
        ...
        for (int i = 0; i < recorderSize/sizeof(short); i ++){
            //printf("test");
            //recorderBuffer
            //__android_log_print(ANDROID_LOG_INFO, "YourApp", "%d ", recorderBuffer[i]);
            int boost = recorderBuffer[i]     *gain;
            if (boost > 32767){
                boost = 32767;
            }
            else if (boost < - 32767){
        ...
        if (pauseCount > 0){
            for (int i = 0; i < pauseCount; i ++){
                uint64_t startIndex = pauseStartIndices[i]/sizeof(short);
                uint64_t  endIndex = pauseEndIndices[i]/sizeof(short);
                if (endIndex <= startIndex){
                    endIndex = recorderSize/sizeof(short);
                }

                memset(&recorderBuffer[startIndex], 0, (endIndex - startIndex)/sizeof(short));
            }
        }
    }
...
void Java_com_example_nativeaudio_NativeAudio_pauseRecording(JNIEnv* env, jclass clazz) {
//    SLresult result;
//    result = (*recorderRecord)->SetRecordState(recorderRecord, SL_RECORDSTATE_PAUSED);
//    if (SL_RESULT_SUCCESS == result) {
//        //recorderSize = (currentTimeInNanos() - timeStart)*22050 * sizeof(short)/1000000000LL;
//    }
//
//    //result = (*recorderBufferQueue)->Clear(recorderBufferQueue);
//    //assert(SL_RESULT_SUCCESS == result);
...
//    pthread_mutex_unlock(&audioEngineLock);
    ...
    // Start byte offset
    if (pauseCount < 19) {
        pauseStartIndices[pauseCount] =
                (currentTimeInNanos() - timeStart) * 22050 * sizeof(short) / 1000000000LL;
        pauseCount++;
    }
...
// set the recording state for the audio recorder
void Java_com_example_nativeaudio_NativeAudio_resumeRecording(JNIEnv* env, jclass clazz)
{
    // Start byte offset
    pauseEndIndices[pauseCount] = (currentTimeInNanos() - timeStart)*22050 * sizeof(short)/1000000000LL;
...
JNICALL  jshortArray Java_com_example_nativeaudio_NativeAudio_recordedData(JNIEnv* env, jclass clazz)
{
    jbyteArray ret = (*env)->NewShortArray(env, recorderSize);

    (*env)->SetShortArrayRegion (env, ret, 0, recorderSize, recorderBuffer);

    return ret;
}

JNICALL  jdouble Java_com_example_nativeaudio_NativeAudio_recordedDuration(JNIEnv* env, jclass clazz)
{
    return (double)(timeStop - timeStart)/1000000ll;
}